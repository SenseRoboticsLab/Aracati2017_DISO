...
bool AerialImage::loadMap(const path &mapFile)
{
  ...
  mapImg_ori = mapImg.clone();
  mapImg_PC = mapImg.clone();
  return true;
}
...
const Mat& AerialImage::getMapPCImg() const
{
  return mapImg_PC;
}

void AerialImage::setMapPCIng(const cv::Mat& img)
{
    mapImg_PC = img;
}

const Mat& AerialImage::getMapOriImg() const
{
  return mapImg_ori;
}
...
void AerialImage::setMapIng(const cv::Mat& img)
{
    mapImg = img;
}
